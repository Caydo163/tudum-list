...
class ListGateway {
    ...
    public function addList($list) {
        if($list->getOwner() == -1) {
            ...
            $this->con->executeQuery($query, array(':name' => array(utf8_decode($list->getName()), PDO::PARAM_STR)) );
        }
        else {
            $query = "INSERT INTO List (name, owner) VALUES (:name, :owner);"; 
            $this->con->executeQuery($query, array(':name' => array(utf8_decode($list->getName()), PDO::PARAM_STR),':owner' => array($list->getOwner(), PDO::PARAM_INT) ) );
        }
    }
